...
def write_report(out, primary, scrub, contrasts_p, K, epochs):
    ...
    action_helps = (
        c["action_helps_lookup_CminusB"] > material
    ...
    L.append(
        "> Reviewer criticism addressed: a planning state is defined by "
    ...
    L.append(
        "- **Models A–F:** majority; state bigram; action bigram "
    ...
    L.append(
        "**1. Results.** See tables above. On the clean Fixed-Init "
    ...
    L.append(
        "**2. Does MLP(z,a) materially beat the action-aware bigram?** "
        ...
        + (
            ...
            "counts.\n"
            if mlp_beats_bigram
            else "The MLP does not exceed the action-conditioned lookup: whatever "
    ...
    if verdict == "yes":
        L.append(
            "> **Tentatively yes (revisit).** Conditioning on the action "
        ...
        L.append(
            "> **No.** " + "; ".join(reasons) + ". The negative result "
...
def main():
    ...
    primary = run_representation(
        "fixed_init",
        os.path.join(fdir, "train.pt"),
        os.path.join(fdir, "val.pt"),
        args.num_codes,
        args.epochs,
        args.seed,
    )
    print("[Exp1] Scrubbed representation...")
    scrub = run_representation(
        "scrubbed",
        os.path.join(sdir, "train.pt"),
        os.path.join(sdir, "val.pt"),
        args.num_codes,
        args.epochs,
        args.seed,
    )
