...
def get_symbolic_states(
    ...
    Optional[Tuple[Tuple[int, Tuple[int, ...]], Tuple[Tuple[int, int, int], ...]]]
]:
...
def gather_state_groups(
    trajectories: List[Trajectory],
) -> Dict[Tuple, List[Tuple[int, Tuple, torch.Tensor]]]:
...
def compute_intrinsic_noise(
    ...
    for i in range(len(all_syms)):
        ...
        for j in range(len(all_syms)):
            if (
                i != j
                and all_t_idx[i] != all_t_idx[j]
                and all_hists[i] != all_hists[j]
                and all_syms[i] == all_syms[j]
            ):
...
def plot_noise_histogram(df: pd.DataFrame, png_path: str) -> None:
    ...
    plt.title(
        "Intrinsic State Noise: Cosine Similarity Distribution",
        fontsize=14,
        fontweight="bold",
    )
