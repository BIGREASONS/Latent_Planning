...
def analyze_condition(
    name, train, val, test, true_train, true_test, K, vq_epochs, tr_epochs, seed
):
    ...
    _, predict = train_code_transition(
        ...
        d_va,
        num_codes=K,
        config=CodeTransitionConfig(epochs=tr_epochs, seed=seed),
    )
    res = run_all_models(
...
def oracle_metrics(true_train, train):
    ...
    return {
        "H_state": s["global_entropy"],
        "det_state": s["det_frac_mass"],
        "H_action": sa["global_entropy"],
        "det_action": sa["det_frac_mass"],
    }
...
def countdown_reference(reports_dir):
    ...
    if os.path.exists(ac_path):
        ...
        out.update(
            {
                "bigram_z": ac["B_bigram"]["top1"],
                "mlp_z": ac["D_mlp_z"]["top1"],
...
def write_report(out_dir, floor, observed, pos, oracle, K):
    ...
    def row(label, key, p=3):
        return (
            f"| {label} | "
            + " | ".join(_f(c[1].get(key), p) for c in cols)
    ...
    L.append(
        "> Question: is Countdown's action-conditioned **H(z'|z,op)=0.99 "
    ...
    L.append("\n## Setup\n")
    L.append(
        "- **Positive control:** deterministic FSM (32 states, 3 actions, "
        "random transition table), each state rendered as `proto[s] + "
        "noise` in 2048-d. Genuine reusable, deterministic `(s,a)→s'`.\n"
    )
    L.append(
        "- **Noise floor:** same FSM walks, but embeddings are pure noise "
        "(no state information).\n"
    )
    L.append(
        "- **Countdown (observed):** Fixed-Init numbers from Experiments "
        "V5.1 / 1, shown for placement.\n"
    )
    L.append(
        "- **Oracle:** dynamics computed on the *true* FSM states (the "
    ...
    # ---- Verdict logic ---- #
    recovered = (
        pos["ami_codes_vs_true"] > 0.6
        and pos["det_action"] >= 0.5
    ...
    if recovered:
        L.append(
            f"**Yes.** On the positive control the VQ codes recover the "
        ...
        L.append(
            f"**Partially.** Positive-control recovery: AMI "
    ...
    L.append(
        f"**Yes.** The pipeline is *capable* of reporting strong "
    ...
    L.append(
        "Placed on each floor→ceiling axis (floor = noise, ceiling = "
        "positive control), Countdown lands:\n"
    )
    L.append(
        f"- **Det-frac(z,a):** floor {_f(floor['det_action'])} → ceiling "
        f"{_f(pos['det_action'])}; Countdown {_f(observed['det_action'])} "
        f"— **{cd_det*100:.0f}%** of the way up."
    )
    L.append(
        f"- **MLP(z,a) acc:** floor {_f(floor['mlp_z_op'])} → ceiling "
        f"{_f(pos['mlp_z_op'])}; Countdown {_f(observed['mlp_z_op'])} "
        f"— **{cd_mlp*100:.0f}%**."
    )
    L.append(
        f"- **H(z'|z,a):** floor {_f(floor['H_action'])} → ceiling "
...
def main():
    ...
    payload = {
        ...
        "noise_floor": floor,
        "oracle_true_states": oracle,
        "countdown_observed": observed,
    }
    ...
    print(
        "\n[Exp2] Positive:",
        {k: round(v, 3) for k, v in pos.items() if isinstance(v, float)},
    )
    print(
        "[Exp2] Floor:",
        {k: round(v, 3) for k, v in floor.items() if isinstance(v, float)},
    )
