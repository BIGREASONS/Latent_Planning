...
def extract_hidden_states(
    model, tokenizer, dataset, target_layer: int, output_file: str
):
    ...
    with torch.no_grad():
        for i, sample in enumerate(tqdm(dataset, desc="Extracting hidden states")):
            ...
            layer_hidden_states = outputs.hidden_states[target_layer].squeeze(
                0
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to trained model checkpoint (or None for base model)",
    )
    parser.add_argument(
        "--data_file", type=str, required=True, help="Path to test.jsonl"
    )
    parser.add_argument(
        "--output_file",
        type=str,
        default="reports/hidden_states.pt",
        help="Output file path (.pt recommended)",
    )
    parser.add_argument(
        "--layer",
        type=int,
        default=-1,
        help="Layer number to extract (-1 for last layer)",
    )
