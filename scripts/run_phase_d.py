...
def train_projection(pos_pairs, neg_pairs, epochs=3, batch_size=128):
    ...
    for ep in range(epochs):
        ...
        for i in range(0, len(all_pairs), batch_size):
            ...
            y = torch.tensor([x["label"] for x in batch], dtype=torch.float32).to(
                device
            )
...
def main():
    ...
    pos_df = pd.DataFrame(
        [{k: v for k, v in p.items() if k not in ["h1", "h2"]} for p in pos_pairs]
    )
    pos_df.to_csv(os.path.join(out, "positive_pairs.csv"), index=False)

    print(
        f"  Generated {len(pos_pairs)} positive pairs and {len(neg_pairs)} negative pairs."
    )
    ...
    retrieval_res = pd.DataFrame(
        [
            {
                "representation": "h (2048)",
                "within_cosine": w_cos_h,
                "between_cosine": b_cos_h,
                "top1": top1_h,
                "top5": top5_h,
            },
            {
                "representation": "z (256)",
                "within_cosine": w_cos_z,
                "between_cosine": b_cos_z,
                "top1": top1_z,
                "top5": top5_z,
            },
        ]
    ...
    X_h_tr, X_z_tr, a_id_tr, a_ops_tr, Y_h_tr, Y_z_tr = build_transition_data(
        model, train_trajs
    )
    X_h_te, X_z_te, a_id_te, a_ops_te, Y_h_te, Y_z_te = build_transition_data(
        model, test_trajs
    )

    tm_h = train_transition(
        X_h_tr, a_id_tr, a_ops_tr, Y_h_tr, hidden_dim=2048, epochs=3
    )
    ...
    for p in probe_res["probe"].unique():
        acc_h = probe_res[
            (probe_res["representation"] == "h (2048)") & (probe_res["probe"] == p)
        ]["accuracy"].iloc[0]
        acc_z = probe_res[
            (probe_res["representation"] == "z (256)") & (probe_res["probe"] == p)
        ]["accuracy"].iloc[0]
    ...
    with open(os.path.join(out, "canonicalization_verdict.md"), "w") as f:
        ...
        if verdict == "ENTANGLED":
            f.write(
                "The planning information already existed in the hidden state but was merely entangled with trajectory history. A lightweight projection successfully removed the history dependence (improving retrieval) while preserving task information.\n"
            )
        else:
            f.write(
                "The planning information is fundamentally absent or inextricably linked to history. Projection failed to separate the symbolic state from the path taken without destroying task performance.\n"
            )
