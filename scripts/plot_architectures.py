...
def main():
    parser = argparse.ArgumentParser(
        description="Plot architectures comparison (Probe A vs Depth)"
    )
    parser.add_argument(
        "--linear_csv", required=True, help="Path to Linear coherence CSV"
    )
    parser.add_argument("--mlp_csv", required=True, help="Path to MLP coherence CSV")
    parser.add_argument(
        "--transformer_csv", required=True, help="Path to Transformer coherence CSV"
    )
    parser.add_argument(
        "--oracle_csv", required=True, help="Path to Oracle coherence CSV"
    )
    ...
    if "identity_state_probe_accuracy" in dfs["MLP"].columns:
        ax.plot(
            dfs["MLP"]["depth"],
            dfs["MLP"]["identity_state_probe_accuracy"],
            label="Identity",
            color=colors["Identity"],
            linestyle=line_styles["Identity"],
            linewidth=2,
        )
    ...
    if "state_probe_accuracy" in dfs["Oracle"].columns:
        ax.plot(
            dfs["Oracle"]["depth"],
            dfs["Oracle"]["state_probe_accuracy"],
            label="Oracle",
            color=colors["Oracle"],
            linestyle=line_styles["Oracle"],
            linewidth=2,
        )
    ...
    for arch in ["Linear", "MLP", "Transformer"]:
        df = dfs[arch]
        if (
            "state_probe_accuracy" in df.columns
            and "identity_state_probe_accuracy" in df.columns
        ):
    ...
    if (
        "state_probe_accuracy" in dfs["Oracle"].columns
        and "identity_state_probe_accuracy" in dfs["Oracle"].columns
    ):
        oracle_gain = (
            dfs["Oracle"]["state_probe_accuracy"]
            - dfs["Oracle"]["identity_state_probe_accuracy"]
        )
        ax2.plot(
            dfs["Oracle"]["depth"],
            oracle_gain,
