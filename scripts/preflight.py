        ...
        with open(train_path, "r", encoding="utf-8") as f:
            ...
            if expected_domain == "game24":
                if target != 24 or len(numbers) != 4:
                    return (
                        False,
                        f"Expected Game24 (target=24, 4 numbers) but got target={target}, {len(numbers)} numbers.",
                    )
...
def main():
    ...
    parser.add_argument(
        "--model",
        required=True,
        help="Model ID (e.g., deepseek-ai/DeepSeek-R1-Distill-Qwen-7B)",
    )
    parser.add_argument(
        "--domain",
        required=True,
    ...
    try:
        ...
        summary.append(
            f"GPU: {gpu_name} ({total_gb:.1f} GB total, {free_gb:.1f} GB free)"
        )
        if free_gb < 14.0:
            print(
                f"[WARN] Less than 14GB free VRAM ({free_gb:.1f}GB). 7B models might OOM even in 4-bit."
            )
    ...
    try:
        ...
        print(
            f"[INFO] Dependencies: torch={torch.__version__}, transformers={transformers.__version__}, "
    ...
    try:
        ...
        if not getattr(tokenizer, "is_fast", False):
            print(
                f"[FAIL] Tokenizer is not fast (tokenizer.is_fast == False). Required for offset mappings."
            )
            sys.exit(1)
        ...
        if (
            ...
            print(
                f"[FAIL] Authentication Error: The model '{args.model}' requires a valid Hugging Face token. "
    ...
    try:
        ...
        if free_gb < 10.0:
            print(
                f"[WARN] Low disk space in current directory: {free_gb:.1f} GB free. Long runs might fill the disk."
            )
        else:
    ...
    if os.path.exists(args.out_dir):
        ...
        collision = [
            a for a in artifacts if os.path.exists(os.path.join(args.out_dir, a))
        ]

        if collision and not args.force:
            print(
                f"[FAIL] Output collision: --out_dir '{args.out_dir}' contains artifacts from a previous run: {collision}"
            )
            print("Use --force to overwrite or specify a new --out_dir.")
            sys.exit(1)
        elif len(os.listdir(args.out_dir)) > 0 and not args.force:
            print(
                f"[FAIL] --out_dir '{args.out_dir}' already exists and is not empty. Use --force to proceed."
            )
            sys.exit(1)
