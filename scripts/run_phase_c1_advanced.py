...
def sample_pairs(flat_h, max_pairs_per_cat=5000):
    ...
    print(
        "  Sampling Same State, Same Depth, Diff Traj & Same State, Diff Depth, Diff Traj..."
    )
    ...
    for t_idx, items in by_traj.items():
        ...
        for i, j in idx[:num_to_sample]:
            ...
            if (
                sym1 != sym2 and d1 == d2
            ):  # usually impossible to have diff state at same depth in SAME traj, but let's check
    ...
    print(
        "  Sampling Diff State, Same Depth, Diff Traj & Diff State, Diff Depth, Diff Traj..."
    )
    # sample randomly
    sampled = 0
    while sampled < max_pairs_per_cat * 5:
        ...
        if sym1 != sym2 and t1 != t2:
            if d1 == d2:
                if add_pair(d1, "DS_SD_DT", h1, h2):
                    sampled += 1
            else:
                if add_pair(d1, "DS_DD_DT", h1, h2):
                    sampled += 1
...
def evaluate_classifiers(flat_h):
    ...
    Xtr, Xte, ytr, yte = train_test_split(
        X_state, y_state_filtered, test_size=0.2, random_state=42
    )
...
def plot_pca(flat_h, out_dir):
    ...
    plt.figure(figsize=(10, 8))
    sns.scatterplot(
        x=X_pca[mask, 0], y=X_pca[mask, 1], hue=syms[mask], palette="tab10", s=10
    )
...
def main():
    ...
    with open(os.path.join(out, "phase_c1_advanced_report.md"), "w") as f:
        ...
        f.write(
            f"- **State Classification Accuracy (Top 50)**: {state_acc*100:.1f}%\n\n"
        )
        ...
        for _, r in df.sort_values(["Depth", "Category"]).iterrows():
            f.write(
                f"| {r['Depth']} | {r['Category']} | {r['Raw Cosine']:.3f} | {r['Centered Cosine']:.3f} | {r['N']} |\n"
            )

        f.write("\n## 4. Representation Score (R)\n")
        f.write(
            "`R = (SS_SD_DT - DS_SD_DT) / (SS_SD_DT - DS_DD_DT)` computed on Centered Cosine.\n\n"
        )
        ...
        for d in sorted(df["Depth"].unique()):
            ...
            if (
                ss_sd is not None
                and ds_sd is not None
                and ds_dd is not None
                and (ss_sd - ds_dd) != 0
            ):
        ...
        if depth_acc > state_acc + 0.20 and mean_r < 0.3:
            ...
            f.write(
                "The representation is fundamentally entangled with token position. PCA and classifiers show depth is the primary axis of variance. Symbolic state retrieval fails across depths.\n"
            )
        elif mean_r > 0.7:
            f.write("**SYMBOLIC STATE DOMINATES.**\n")
            f.write(
                "The representation preserves symbolic state across reasoning depths. R score is high.\n"
            )
        else:
