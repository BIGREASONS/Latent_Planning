...
def maybe_generate_data(
    data_dir: str, sizes, domain: str = "countdown", base_seed: int = 42
) -> None:
...
def _generate_game24_disjoint(data_dir, sizes, base_seed):
    ...
    if total_req > total_available:
        ...
        print(
            f"  [INFO] Game24 pool size is {total_available}. Scaling requested splits down to: train={req_train}, val={req_val}, test={req_test}"
        )
    ...
    if need_gen:
        ...
        print(
            f"  [INFO] Generating mutually exclusive Game24 splits: train={req_train}, val={req_val}, test={req_test}"
        )
...
def build_split_trajectories(
    ...
    for split in splits:
        ...
        if trajectories_dir and os.path.exists(
            os.path.join(trajectories_dir, f"{split}.pt")
        ):
            print(
                f"  [{split}] Loading existing trajectories from {trajectories_dir}..."
            )
            ...
            t = build_trajectories(
                model, tokenizer, problems, layer=layer, batch_size=batch_size
            )
            os.makedirs(os.path.dirname(target_path), exist_ok=True)
            save_trajectories(t, target_path)
    ...
    if "val" in trajs:
        ...
        trajs["val"] = [
            t for t in trajs["val"] if get_problem_hash(t) not in train_hashes
        ]
        val_hashes = set(get_problem_hash(t) for t in trajs["val"])
        dropped_val = orig_val - len(trajs["val"])
        if dropped_val > 0:
            print(
                f"  [WARNING] Dropped {dropped_val} leaking trajectories from Val set."
            )
    ...
    if "test" in trajs:
        ...
        trajs["test"] = [
            t
            for t in trajs["test"]
            if get_problem_hash(t) not in train_hashes
            and get_problem_hash(t) not in val_hashes
        ]
        dropped_test = orig_test - len(trajs["test"])
        if dropped_test > 0:
            print(
                f"  [WARNING] Dropped {dropped_test} leaking trajectories from Test set."
            )
    ...
    if "test_ood" in trajs:
        ...
        if dropped_ood > 0:
            print(
                f"  [WARNING] Dropped {dropped_ood} leaking trajectories from Test OOD set."
            )
...
def generate_master_report(
    ...
    final_val_mse = (
        transition_history[-1].get("eval_loss") if transition_history else None
    )
    ...
    chance_map = {
        row["probe"][0]: c
        for row, c in zip(
            [r for _, r in probe_df.iterrows()],
    ...
    if max_depth_avail:
        ...
        def _get_metrics(df, id_df):
            if df is None or len(df[df["depth"] == 1]) == 0:
    ...
    if info_present and survives_beyond_3:
        verdict = (
            "**Promising.** Hidden states encode task-relevant structure "
        ...
        verdict = (
            "**Mixed / representation-limited bottleneck.** The "
        ...
        verdict = (
            "**Inconclusive.** Latents stay self-consistent under rollout "
        ...
        verdict = (
            "**Not yet.** Hidden states show weak task information and "
    ...
    lines.append(
        ...
        + (
            "representation"
            if (not info_present and survives_beyond_3)
            else (
                "dynamics"
                if (info_present and not survives_beyond_3)
                else (
                    "neither (proceed)"
                    if (info_present and survives_beyond_3)
                    else "both"
...
def main():
    ...
    parser.add_argument(
        "--cap", type=int, default=None, help="Max problems per split (for quick runs)"
    )
    parser.add_argument(
        "--generate",
        nargs="+",
        ...
        metavar="SIZE",
        default=None,
        help="Generate data if missing. Defaults to [5000, 500, 1000, 500] for countdown, [1000, 150, 196] for game24.",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=32,
        help="Batch size for extracting hidden states",
    )
    parser.add_argument(
        "--load_in_4bit",
        action="store_true",
    ...
    parser.add_argument(
        "--extract_only", action="store_true", help="Only generate trajectories"
    )
    parser.add_argument(
        "--trajectories_dir",
        type=str,
        default=None,
        help="Use pre-extracted trajectories from this directory",
    )
    parser.add_argument(
        "--seed", type=int, default=42, help="Base seed for reproducibility"
    )
    ...
    if args.generate:
        ...
        maybe_generate_data(
            args.data_dir, args.generate, domain=args.domain, base_seed=args.seed
        )
    ...
    print(
        "[3/6] Training latent transition models (Action-Conditioned + Action-Blind)..."
    )
    tcfg = TransitionTrainConfig(
    ...
    save_coherence(
        coh_df,
        os.path.join(out, "coherence_action_depth.csv"),
    ...
    save_coherence(
        coh_blind_df,
        os.path.join(out, "coherence_blind_depth.csv"),
    ...
    coh_shuffled_df = evaluate_coherence(
        ...
        max_depth=args.max_depth,
        domain=args.domain,
        rollout_mode="shuffled",
    )
    save_coherence(
        coh_shuffled_df,
        os.path.join(out, "coherence_shuffled_depth.csv"),
    ...
    coh_constant_df = evaluate_coherence(
        ...
        max_depth=args.max_depth,
        domain=args.domain,
        rollout_mode="constant",
    )
    save_coherence(
        coh_constant_df,
        os.path.join(out, "coherence_constant_depth.csv"),
    ...
    if "test_ood" in trajs:
        ...
        save_coherence(
            coh_ood_df,
            os.path.join(out, "coherence_ood_depth.csv"),
    ...
    d_model_val = getattr(
        ...
        "d_model",
        getattr(
            tmodel_action, "hidden_dim", getattr(tmodel_action, "mlp_hidden_dim", "N/A")
        ),
    )
    bottleneck_val = (
        tcfg.mlp_hidden_dim
        if tcfg.transition_arch != "transformer"
        else getattr(tmodel_action, "d_model", tcfg.mlp_hidden_dim // 2)
    )
    layers_val = (
        0
        if tcfg.transition_arch == "linear"
        else (2 if tcfg.transition_arch == "transformer" else 1)
    )  # simple mapping
    ...
    meta = {
        ...
        "n_train": len(trajs["train"]),
        "n_val": len(trajs["val"]),
        "n_test": len(trajs["test"]),
        "smoke": args.smoke,
        "domain": args.domain,
    ...
    for name in [
        ...
        "coherence_action_depth.png",
        "coherence_blind_depth.png",
