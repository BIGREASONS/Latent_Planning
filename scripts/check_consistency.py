...
ARCHIVE_DIR = (
    "C:/Users/singh/OneDrive/Documents/latent_planning/research_archive/experiments"
)
    ...
    for exp in experiments:
        ...
        if os.path.exists(md_b_path):
            ...
            if expected_str_id in md_b:
                print(
                    f"  [OK] Depth 1 Identity Cosine {expected_str_id} found in Phase B report."
                )
            else:
                print(
                    f"  [FAIL] Depth 1 Identity Cosine {expected_str_id} NOT found in Phase B report!"
                )
            ...
            if expected_str_action in md_b:
                print(
                    f"  [OK] Depth 1 Action Cosine {expected_str_action} found in Phase B report."
                )
            else:
                print(
                    f"  [FAIL] Depth 1 Action Cosine {expected_str_action} NOT found in Phase B report!"
                )
                all_consistent = False
        ...
        if os.path.exists(md_a_path):
            ...
            if expected_str_action in md_a:
                print(
                    f"  [OK] Depth 1 Action Cosine {expected_str_action} found in Phase A report."
                )
            else:
                print(
                    f"  [FAIL] Depth 1 Action Cosine {expected_str_action} NOT found in Phase A report!"
                )
                all_consistent = False
