...
def main():
    ...
    parser.add_argument(
        "--mlp_hidden_dim", type=int, default=512
    )  # Use 512 for MLP inner dim, and Transformer d_model
    parser.add_argument("--transformer_layers", type=int, default=2)
    parser.add_argument("--transformer_heads", type=int, default=8)
    args = parser.parse_args()

    linear_model = LinearTransitionModel(
        hidden_dim=args.hidden_dim,
        mlp_hidden_dim=args.mlp_hidden_dim,
        op_embed_dim=args.op_embed_dim,
    )
    mlp_model = TransitionModel(
        hidden_dim=args.hidden_dim,
        mlp_hidden_dim=args.mlp_hidden_dim,
        op_embed_dim=args.op_embed_dim,
    )
    ...
    print(
        f"Transformer Transition Model params: {count_parameters(transformer_model):,}"
    )
