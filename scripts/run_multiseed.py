...
def main():
    parser = argparse.ArgumentParser(
        description="Multi-seed runner for transition models"
    )
    ...
    print(
        "This runner explicitly fixes the dataset and trajectories while varying the training seed."
    )
    ...
    for arch in args.archs:
        for seed in args.seeds:
            ...
            if all(os.path.exists(f) for f in expected_files):
                print(
                    f"[{arch} - seed {seed}] All completion artifacts found, skipping execution to resume."
                )
            ...
            def _get_metrics(csv_path):
                try:
                    ...
                    sem_gain = (
                        d1["state_probe_accuracy"] - d1["identity_state_probe_accuracy"]
                    )
            ...
            true_sg, true_cos, true_mse, true_mcc, true_acc = _get_metrics(
                f"{out_dir}/coherence_action_depth.csv"
            )
            shuf_sg, _, _, _, _ = _get_metrics(
                f"{out_dir}/coherence_shuffled_depth.csv"
            )
            const_sg, _, _, _, _ = _get_metrics(
                f"{out_dir}/coherence_constant_depth.csv"
            )
            blind_sg, _, _, _, _ = _get_metrics(f"{out_dir}/coherence_blind_depth.csv")
            oracle_sg, _, _, _, oracle_acc = _get_metrics(
                f"{out_dir}/coherence_oracle_depth.csv"
            )

            oracle_gap = (
                oracle_acc - true_acc
                if not np.isnan(oracle_acc) and not np.isnan(true_acc)
                else np.nan
            )
    ...
    if os.path.exists(csv_path):
        ...
        df = pd.concat([old_df, new_df]).drop_duplicates(
            subset=["Architecture", "Seed"], keep="last"
        )
    else:
        df = new_df

    # Sort deterministically
    df["Arch_cat"] = pd.Categorical(
        df["Architecture"], categories=["linear", "mlp", "transformer"], ordered=True
    )
    ...
    print(
        "| Architecture | True Action SG | Shuffled Action SG | Constant Action SG | Blind SG | Oracle Gap |"
    )
    print("|---|---|---|---|---|---|")
    for arch in args.archs:
        ...
        print(
            f"| {arch.capitalize():<12} | {t_sg:<16} | {s_sg:<18} | {c_sg:<18} | {b_sg:<10} | {o_gap:<12} |"
        )
