...
def test_transition_matrix_stochastic():
    ...
    for _ in range(10):
        ...
        trajs.append(
            DiscreteTrajectory(
                codes=codes,
                op_ids=torch.zeros(5, dtype=torch.long),
                operands=torch.zeros(5, 2),
                numbers=[1],
                target=1,
            )
        )
...
def test_transition_matrix_empty_rows_uniform():
    ...
    trajs = [
        DiscreteTrajectory(
            ...
            op_ids=torch.zeros(1, dtype=torch.long),
            operands=torch.zeros(1, 2),
            numbers=[1],
            target=1,
        )
    ]
...
def test_conditional_entropy_deterministic_is_zero():
    ...
    trajs = [
        DiscreteTrajectory(
            ...
            op_ids=torch.zeros(3, dtype=torch.long),
            operands=torch.zeros(3, 2),
            numbers=[1],
            target=1,
        )
    ]
...
def test_save_entropy_report(tmp_path):
    ...
    trajs = [
        DiscreteTrajectory(
            ...
            op_ids=torch.zeros(4, dtype=torch.long),
            operands=torch.zeros(4, 2),
            numbers=[1],
            target=1,
        )
    ]
