...
% Define a restrained gray for secondary text
\definecolor{metadatagray}{gray}{0.35}

% 1. CUSTOM MAKETITLE: professional preprint front matter
\makeatletter
\def\@maketitle{%
  \newpage
  \null
  % --- Preprint header (understated, right-aligned) ---
  \begingroup
    \raggedleft
    \small\color{metadatagray}%
    Preprint\enspace$\cdot$\enspace September 2026\par
  \endgroup
  \vskip 1.2em%
  % --- Title ---
  {\centering
  ...
  % --- Author block ---
  {\centering
   \large\bfseries Vaibhav Singh\par
   \vskip 0.3em%
   \small\normalfont Independent Researcher\par
   \vskip 0.15em%
   \small\normalfont\color{metadatagray}%
   \href{mailto:singhvaibhavip@gmail.com}{singhvaibhavip@gmail.com}\par
  }%
  \vskip 1.0em%
  % --- Thin rule separating author block from abstract ---
  {\centering\rule{0.4\textwidth}{0.4pt}\par}%
  \vskip 0.8em%
}
\makeatother

% 2. CAPTION: slightly smaller caption text
\makeatletter
...
% 3. ABSTRACT: keep IEEEtran default rendering (bold/italic is standard for IEEE conference)

% 4. BIBLIOGRAPHY: compact spacing
...
\title{Beyond Geometric Similarity: Measuring Semantic Fidelity\\in Latent LLM Transitions}

\author{}% Author block is rendered manually inside \@maketitle
...
\toprule
Work & Pred. & Probe & Oracle & Ctrl. & Geo/Sem. \\
\midrule
...
\subsection{Teacher Representation}
We extract the hidden state from the final layer (`-1') of the frozen teacher model, \textbf{TinyLlama/TinyLlama-1.1B-Chat-v1.0}. The use of a small-scale model enables high-throughput trajectory extraction while retaining sufficient representational capacity for deductive reasoning tasks.

\subsection{Action Representation}
Each action $a_t$ is represented by concatenating a 16-dimensional learned operator embedding with its two normalized numeric operands, yielding an 18-dimensional action vector. The identical action representation is used across all transition architectures.

\subsection{Transition Dimensions}
Linear, MLP, and Transformer transitions operate over the 2048-dimensional hidden states. The Linear transition uses a direct low-rank linear projection. The MLP uses a single 512-dimensional hidden layer with a ReLU activation. The Transformer uses a 2-layer, 8-head bottleneck self-attention configuration with $d_{model}=256$, matching the parameter count of the MLP baseline.

\subsection{Probe}
Probes use regularized logistic regression with the L-BFGS solver, $C=1.0$, and a maximum of 2000 iterations, trained on approximately 15,000 sampled state representations from the 5,000 training trajectories.

\subsection{Seeds and Reproducibility}
To ensure robustness, all architectures were trained and evaluated across three independent random initialization seeds: 42, 43, and 44. This produces $3 \times 3 = 9$ discrete experimental conditions. The canonical result artifacts and analysis pipeline are archived in the repository; model-checkpoint availability is documented separately due to storage constraints. The canonical repository preserving the software and experimental outputs for the experiments reported in this manuscript is available at \url{https://github.com/BIGREASONS/Latent_Planning} (Commit: \texttt{d86ac1556a191e2c70aa6be6260d6387073d665e}).
...
To ensure generalization, datasets were explicitly deduplicated to prevent test-set tasks or identical reasoning sub-trajectories from leaking into the training corpus. After trajectory-level deduplication, the effective validation and test sets contained 491 and 970 problems, respectively.
...
\midrule
Linear & $0.1944 \pm 0.0658$ & $-0.0339 \pm 0.0656$ \\
MLP & $0.1789 \pm 0.0155$ & $-0.0183 \pm 0.0155$ \\
Transformer & $0.1860 \pm 0.0137$ & $-0.0254 \pm 0.0137$ \\
\bottomrule
\end{tabular}
\end{table}
...
The results provide little evidence that the tested transitions derive semantic benefit from the supplied action. The True Action condition performs comparably to the Shuffled and Constant control settings, with negative SG in almost all evaluations.

\subsection{Statistical Analysis}
We formally tested whether the architecture choice systematically affects the Oracle Gap. The comprehensive statistical tests are reported in Table \ref{tab:stats}. A Welch's ANOVA yields $F(2,3.59)=1.160$ with $p=0.409$, which did not provide statistically significant evidence of differences in mean Oracle Gap across architectures. Holm-adjusted pairwise Welch $t$-tests likewise show no statistically significant differences. To verify whether initialization variance masked architecture effects, we additionally conducted a paired sensitivity analysis blocking by random seed; this approach also yielded no statistically significant architecture-level differences ($F(2,4)=0.604$, $p=0.590$). Although large effect sizes were observed (e.g., Cohen's $d = -1.210$ for MLP vs Transformer), these effect-size estimates are exploratory and should be interpreted cautiously given the very small number of independent seeds. The study does not provide statistically significant evidence of architecture-level differences, and the low seed count ($n=3$) makes the result inconclusive regarding architectural equivalence.
...
\begin{tabular}{@{}lrrrr@{}}
\toprule
Comparison / Test & Stat. & $p$ & Corr.\ $p$ & Effect \\
\midrule
Welch ANOVA & $F{=}1.160$ & $0.409$ & --- & --- \\
MLP vs Trans. & $-1.481$ & $0.2137$ & $0.6411$ & $d{=}{-}1.210$ \\
Linear vs MLP & $0.989$ & $0.4178$ & $0.8355$ & $d{=}0.808$ \\
Linear vs Trans. & $0.539$ & $0.6402$ & $0.8355$ & $d{=}0.440$ \\
\bottomrule
\end{tabular}
\end{table}
...
\bottomrule
\multicolumn{5}{p{0.95\textwidth}}{\small \textsuperscript{*}Exploratory quantities derived from the earlier evaluation pipeline; not included in the primary Phase 3 statistical analysis.}
\end{tabular}
\end{table*}
...
High-dimensional LLM representations contain structural scaffolding that does not pertain to the local reasoning step. This pattern is consistent with MSE or cosine objectives favoring preservation of high-magnitude background components. The resulting state is geometrically close, yet deficient in the sensitive subspace where the actual deductive operation is encoded.
...
\small
\setlength{\tabcolsep}{3pt}
\begin{tabular}{@{}p{3.6cm}p{1.6cm}l@{}}
\toprule
...
Table \ref{tab:raw_results_app} presents the complete set of raw metrics obtained across all nine audited experimental runs in Phase 3. Table \ref{tab:action_controls_app} presents the corresponding action-control ablation results for Semantic Gain.
...
\midrule
Linear & 42 & -0.0196 & 0.1603 & 0.1799 & 0.9410 \\
Linear & 43 & -0.0644 & 0.1606 & 0.2250 & 0.9414 \\
Linear & 44 & -0.0178 & 0.1606 & 0.1784 & 0.9409 \\
MLP & 42 & -0.0255 & 0.1606 & 0.1861 & 0.9466 \\
MLP & 43 & -0.0149 & 0.1606 & 0.1755 & 0.9468 \\
MLP & 44 & -0.0144 & 0.1606 & 0.1750 & 0.9450 \\
Trans. & 42 & -0.0206 & 0.1606 & 0.1812 & 0.9464 \\
Trans. & 43 & -0.0242 & 0.1606 & 0.1848 & 0.9459 \\
Trans. & 44 & -0.0314 & 0.1606 & 0.1920 & 0.9475 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[t]
\centering
\caption{Phase 3 Action-Control Semantic Gain}
\label{tab:action_controls_app}
\small
\setlength{\tabcolsep}{3pt}
\begin{tabular}{@{}lrrrrr@{}}
\toprule
Arch. & Seed & True SG & Shuffled SG & Constant SG & Blind SG \\
\midrule
Linear & 42 & -0.0196 & -0.0227 & -0.0206 & -0.0405 \\
Linear & 43 & -0.0644 & -0.0670 & -0.0683 & -0.0271 \\
Linear & 44 & -0.0178 & -0.0188 & -0.0170 & -0.0407 \\
MLP & 42 & -0.0255 & -0.0253 & -0.0180 & -0.0237 \\
MLP & 43 & -0.0149 & -0.0144 & -0.0152 & -0.0201 \\
MLP & 44 & -0.0144 & -0.0144 & -0.0162 & -0.0137 \\
Trans. & 42 & -0.0206 & -0.0211 & -0.0245 & -0.0237 \\
Trans. & 43 & -0.0242 & -0.0263 & -0.0263 & -0.0198 \\
Trans. & 44 & -0.0314 & -0.0322 & -0.0253 & -0.0250 \\
\bottomrule
\end{tabular}
\end{table}
